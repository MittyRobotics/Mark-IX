...
TKOShooter::TKOShooter(int port1, int port2) :
	j1(port1, CANJaguar::kSpeed), j2(port2, CANJaguar::kPercentVbus)
{
...
bool TKOShooter::ReadyToFire()
{ //TODO: use setpoint and getspeed to determine whether or not it is up to speed
	//setpoint is in rpm, and getspeed is in rpm
	if (!(setpoint <= 250) && (-j1.GetSpeed() >= (setpoint - (error / 2 + error / 5)))
			&& (-j1.GetSpeed() <= (setpoint + error / 2)))
		return true;
	return false;
}
